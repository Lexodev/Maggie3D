...
{
  printf("[MAGGIE3D] Dump draw data\n");
...
{
#if _TRACE_MAGGIE_ == 1
  printf("[MAGGIE3D] Dump Maggie registers\n");
...
VOID M3D_SwapVertex(M3D_Vertex *va, M3D_Vertex *vb)
{
  ...
  DDbug(printf("[MAGGIE3D] M3D_SwapVertex\n");)
...
ULONG M3D_CheckTriangleType(M3D_Context *context, M3D_Triangle *triangle, M3D_DrawData *draw_data)
{
  ...
  if (triangle->v1.x == triangle->v2.x && triangle->v2.x == triangle->v3.x) {
    DDbug(printf("[MAGGIE3D] - Rejected because x1=x2=x3\n");)
    return TRI_REJECTED;
  }
  if (triangle->v1.y == triangle->v2.y && triangle->v2.y == triangle->v3.y) {
    DDbug(printf("[MAGGIE3D] - Rejected because y1=y2=y3\n");)
    return TRI_REJECTED;
  }
...
ULONG M3D_CheckQuadType(M3D_Context *context, M3D_Quad *quad, M3D_DrawData *draw_data)
{
  ...
  if (quad->v1.x == quad->v2.x && quad->v2.x == quad->v3.x && quad->v3.x == quad->v4.x) {
    DDbug(printf("[MAGGIE3D] - Rejected because x1=x2=x3=x4\n");)
    return QUAD_REJECTED;
  }
  if (quad->v1.y == quad->v2.y && quad->v2.y == quad->v3.y && quad->v3.y == quad->v4.y) {
    DDbug(printf("[MAGGIE3D] - Rejected because y1=y2=y3=y4\n");)
    return QUAD_REJECTED;
  }
  // Trivial rejection
  if (quad->v1.x < draw_data->left_clip && quad->v2.x < draw_data->left_clip && quad->v3.x < draw_data->left_clip && quad->v4.x < draw_data->left_clip) {
    DDbug(printf("[MAGGIE3D] - Rejected because x1,x2,x3 & x4 < left_clip\n");)
    return QUAD_REJECTED;
  }
  if (quad->v1.x >= draw_data->right_clip && quad->v2.x >= draw_data->right_clip && quad->v3.x >= draw_data->right_clip && quad->v4.x >= draw_data->right_clip) {
    DDbug(printf("[MAGGIE3D] - Rejected because x1,x2,x3 & x4 >= right_clip\n");)
    return QUAD_REJECTED;
  }
  if (quad->v1.y < draw_data->top_clip && quad->v2.y < draw_data->top_clip && quad->v3.y < draw_data->top_clip && quad->v4.y < draw_data->top_clip) {
    DDbug(printf("[MAGGIE3D] - Rejected because y1,y2,y3 & y4 < top_clip\n");)
    return QUAD_REJECTED;
  }
  // Order in clock wise
  ...
  DDbug(printf("[MAGGIE3D] - Quad in clock wise\n");)
  ...
  DDbug(printf("[MAGGIE3D] - Min Y=%f (vertex %d)\n", miny, minv);)
  if (miny >= draw_data->bottom_clip) {
    DDbug(printf("[MAGGIE3D] - Rejected because y1,y2,y3 & y4 >= bottom_clip\n");)
    return QUAD_REJECTED;
  }
  ...
  DDbug(printf("[MAGGIE3D] - Quad ordered\n");)
  DDbug(M3D_DumpQuad(quad);)
  type = QUAD_GENERIC;
...
VOID M3D_DrawShadedQuad(M3D_Context *context, M3D_Quad *quad, M3D_DrawData *draw_data, ULONG type)
{
  ...
  if (type == QUAD_FLATTOP) {
    DDbug(printf("[MAGGIE3D] M3D_DrawQuadFlatTop\n");)
    if (context->states & M3D_GOURAUD) {
    ...
    DDbug(printf("[MAGGIE3D] M3D_DrawQuadFlatBottom\n");)
    if (context->states & M3D_GOURAUD) {
    ...
    DDbug(printf("[MAGGIE3D] M3D_DrawQuadFlatBoth\n");)
    if (context->states & M3D_GOURAUD) {
    ...
    DDbug(printf("[MAGGIE3D] M3D_DrawQuadGeneric\n");)
    if (context->states & M3D_GOURAUD) {
...
VOID M3D_DrawTexturedQuad(M3D_Context *context, M3D_Quad *quad, M3D_DrawData *draw_data, ULONG type)
{
  ...
  // Render the quad depending on his type
  if (type == QUAD_FLATTOP) {
    DDbug(printf("[MAGGIE3D] M3D_DrawTexturedFlatTopQuad\n");)
    if (context->states & M3D_GOURAUD) {
    ...
    DDbug(printf("[MAGGIE3D] M3D_DrawTexturedFlatBottomQuad\n");)
    if (context->states & M3D_GOURAUD) {
    ...
    DDbug(printf("[MAGGIE3D] M3D_DrawTexturedFlatBothQuad\n");)
    if (context->states & M3D_GOURAUD) {
    ...
    DDbug(printf("[MAGGIE3D] M3D_DrawTexturedGenericQuad\n");)
    if (context->states & M3D_GOURAUD) {
...
LONG M3D_DrawTriangle(M3D_Context *context, M3D_Triangle *triangle)
{
  ...
  DDbug(printf("[MAGGIE3D] M3D_DrawTriangle\n");)
  DDbug(M3D_DumpTriangle(triangle);)
  if (context != NULL) {
...
LONG M3D_DrawTriangleArray(M3D_Context *context, M3D_Triangle *triangles, ULONG count)
{
  ...
  DDbug(printf("[MAGGIE3D] M3D_DrawTriangleArray\n");)
  if (context != NULL) {
    if (context->maggie_available) {
      maggie->mode = context->mode;
      maggie->modulo = context->drawregion.bpp;
...
LONG M3D_DrawTriangleList(M3D_Context *context, M3D_Triangle **triangles, ULONG count)
{
  ...
  DDbug(printf("[MAGGIE3D] M3D_DrawTriangleList\n");)
  if (context != NULL) {
    if (context->maggie_available) {
      maggie->mode = context->mode;
      maggie->modulo = context->drawregion.bpp;
...
LONG M3D_DrawQuad(M3D_Context *context, M3D_Quad *quad)
{
  ...
  DDbug(printf("[MAGGIE3D] M3D_DrawQuad\n");)
  DDbug(M3D_DumpQuad(quad);)
  if (context != NULL) {
    if (context->maggie_available) {
      maggie->mode = context->mode;
...
LONG M3D_DrawNormalSprite(M3D_Context *context, M3D_Sprite *sprite, LONG xpos, LONG ypos)
{
  ...
  DDbug(printf("[MAGGIE3D] Sprite clipping %d,%d -> %d,%d\n", clip_left, clip_top, clip_right, clip_bottom);)
  // Apply zoom factor
  ...
  if (xpos >= clip_right || ypos >= clip_bottom || (xpos + dx) < clip_left || (ypos + dy) < clip_top) {
    DDbug(printf("[MAGGIE3D] Sprite out of clipping region\n");)
    return M3D_SUCCESS;
  }
  // Setup Maggie registers
  ...
  DDbug(printf("[MAGGIE3D] Draw sprite dx=%d, dy=%d, ui=%f, vi=%f, du=%f, dv=%f\n", dx, dy, ui, vi, du, dv);)
  // Destination address
...
LONG M3D_DrawSprite(M3D_Context *context, M3D_Sprite *sprite, LONG xpos, LONG ypos)
{
  ...
  DDbug(printf("[MAGGIE3D] M3D_DrawSprite at %d,%d\n", xpos, ypos);)
...
LONG M3D_LockHardware(M3D_Context *context)
{
  if (context != NULL) {
    if (context->drawregion.bitmap != NULL) {
      ...
      DDbug(printf("[MAGGIE3D] Hardware locked with buffer address 0x%X\n", context->drawregion.data);)
      OwnBlitter();
      return M3D_SUCCESS;
    }
    return M3D_NOBITMAP;
...
VOID M3D_UnlockHardware(M3D_Context *context)
{
  if (context != NULL) {
    DisownBlitter();
    DDbug(printf("[MAGGIE3D] Hardware unlocked\n");)
  }
}
...
LONG M3D_ClearDrawRegion(M3D_Context *context, ULONG color)
{
  ...
  if (context != NULL) {
    if (context->drawregion.data != NULL) {
      ...
      DDbug(printf("[MAGGIE3D] Clear region %d,%d -> %d,%d (%d)\n",
