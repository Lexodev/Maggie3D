 ...
 * @version 1.2 March 2024 (update: 10/03/2024)
 */
...
APTR M3D_AllocMemoryBloc(ULONG size, ULONG attributes, ULONG align)
{
  ...
  Dbug(kprintf("[MAGGIE3D] Memory allocation 0x%lx (0x%lx) of %ld bytes (align %ld)\n", base, memory, size, align);)
  return memory;
}
...
VOID M3D_FreeMem(APTR address)
{
  ...
  // Memory bloc found
  if (node != NULL) {
    Dbug(kprintf("[MAGGIE3D] Memory release 0x%lx of %ld bytes\n", node->base_address, node->bloc_size);)
...
VOID M3D_ReleaseMem()
{
  ...
  node = M3D_Memory.head;
  while (node != NULL) {
    Dbug(kprintf("[MAGGIE3D] Releasing memory bloc 0x%lx of %ld bytes\n", node->base_address, node->bloc_size);)
    FreeMem(node->base_address, node->bloc_size);
    M3D_Memory.head = node->next;
    FreeMem(node, sizeof(M3D_MemoryNode));
    node = M3D_Memory.head;
