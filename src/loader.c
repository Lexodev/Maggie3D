 ...
 * @version 1.3 April 2024 (updated: 17/03/2024)
 */
...
M3D_TextureFile *M3D_LoadDDSTexture(LONG *error, STRPTR file_name)
{
  ...
  Dbug(kprintf("[MAGGIE3D] M3D_LoadDDSTexture %s \n", file_name);)
  ...
  if (file_handle != 0) {
    ...
    Dbug(kprintf(
        "[MAGGIE3D] DDS header => width=%ld, height=%ld, depth=%ld, data_size=%ld \n",
...
M3D_TextureFile *M3D_LoadBMPTexture(LONG *error, STRPTR file_name)
{
  ...
  Dbug(kprintf("[MAGGIE3D] M3D_LoadBMPTexture %s \n", file_name);)
  file_handle = Open(file_name, MODE_OLDFILE);
  if (file_handle != 0) {
    ...
    Dbug(kprintf("[MAGGIE3D] BMP image_offset=%ld, dib_size=%ld \n", image_offset, dib_header.size);)
    ...
    if (texfile->depth == 8) {
      Dbug(kprintf("[MAGGIE3D] Load the color map\n");)
    ...
    Dbug(kprintf(
        "[MAGGIE3D] BMP header => width=%ld, height=%ld, depth=%ld, data_size=%ld \n",
        texfile->width, texfile->height, texfile->depth, texfile->data_size
    );)
    // Reset file pointer
    bytes_read = Seek(file_handle, 0, OFFSET_BEGINNING);
    ...
    Close(file_handle);
    // Mirror picture
    Dbug(kprintf("[MAGGIE3D] Mirror picture\n");)
    ...
    if (texfile->depth != 8) {
      ...
      Dbug(kprintf("[MAGGIE3D] Remap to Big endian format\n");)
      picture_data = (UBYTE *) texfile->data;
      if (texfile->depth == 16) {
        height = texfile->height;
        while (height--) {
